    ...
    domain.run(function () {
      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if ( cache.length ) {
            var headers = JSON.parse(cache[0].headers);

            Object.keys(headers).forEach(function (key) {
              res.setHeader(key, headers[key]);
            });
            res.contentType(cache[0].type || "text/html");
            // Override X-Cache headers
          ...
          /** otherwise, cache request **/
            ...
            res.send = function (body) {
              ...
              Object.keys(headerNames).forEach(function (headerName) {
                headers[headerNames[headerName]] = currentHeaders[headerName];
              });
