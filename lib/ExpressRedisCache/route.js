    ...
    domain.run(function () {
      ...
      middleware = function expressRedisCache_Middleware (req, res, next) {
        ...
        self.get(name, domain.bind(function (error, cache) {
          ...
          if ( cache.length ) {
            if(cache[0].headers !==undefined && cache[0].headers ['content-cache'] !==undefined){
              res.header('content-cache',cache[0].headers['content-cache']);
            }
            res.header('content-type',cache[0].type || "text/html");
